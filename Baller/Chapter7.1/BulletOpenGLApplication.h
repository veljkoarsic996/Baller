...
public:
	...
	void createPortal();
	void DrawStrokeText(char* string, int x, int y, const float r, const float g, const float b);
	void DrawText(char* string, float x, float y, const float r, const float g, const float b);
	void InitGround();
	void Initialize();
	void restartPos();
	void restartGame();
	void checkKeyFlags();
	void restartJump(float p_deltaTime);
	...
	void drawPortalWithTexture(const btVector3& halfSize);
	...
	void DrawPortal(btScalar* transform, const btCollisionShape* pShape);
...
/*ADD*/		void DrawCylinder(const btScalar &radius, const btScalar &halfHeight);
	...
	GameObject* makeKey(btCollisionShape* pShape, const float& mass, const btVector3& color, const btVector3& initialPosition);

	GameObject* CreateGroundObject(btCollisionShape* pShape, const float& mass, const btVector3& color, const btVector3& initialPosition);

	GameObject* makePortal(btCollisionShape* pShape, const float& mass, const btVector3& color, const btVector3& initialPosition, const btQuaternion& initialRotation);
	...
	//void destroyKeyObject(GameObject* pBody);

	void destroyKeyObject(btRigidBody* pBody);
...
protected:
	...
	CollisionPairs m_keyPairsLastUpdate;
};
#endif
